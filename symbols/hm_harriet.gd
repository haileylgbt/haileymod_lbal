...
func init(modloader: Reference, params):
    ...
    self.texture = load_texture("res://haileymod/symbols/harriet.png")
    self.name = "Harriet"
...
func add_conditional_effects(symbol, adjacent):
    for i in adjacent:
        ...
        symbol.add_effect_for_symbol(i, effect().if_type("hm_flag").change_type("hm_lesbian_flag").animate("shake"))
        symbol.add_effect_for_symbol(i, effect().if_type("hm_hailey").set_value_bonus(1).animate("bounce"))
